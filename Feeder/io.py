...
def load_case(bus_path, line_path, shunt_path):
    ...
    for _, row in bus_info.iterrows():
        ...
        bus_dict[bus_id] = {
            "type": row["bus_type"],
            "P_MW": clean(row["P_load"]),
            "Q_load": clean(row["Q_load"]),
            "P_gen": clean(row['P_gen']),
            'Q_gen': clean(row['Q_gen']),
            "Q_min": clean(row["Q_min"]),
            "Q_max": clean(row["Q_max"]),
            "V_spec": clean(row ["V_spec_pu"]),
            "Normal Vmax (pu)": clean(row["Normal V_max (pu)"]),
            "Normal Vmin (pu)": clean(row["Normal V_min (pu)"]),
            "Emergency Vmax (pu)": clean(row["Emergency V_max (pu)"]),
            "Emergency Vmin (pu)": clean(row["Emergency V_min (pu)"]),
            'shunt_type': 'none',
            'status': 'off',
            'G-shunt(pu)': 0,
            'B-shunt(pu)': 0
    ...
    for _, row in shunt_info.iterrows():
        ...
        if row["Status"] == 'on' or row['Status']==1:
            bus_dict[bus_id]['shunt_type']= row['model']
