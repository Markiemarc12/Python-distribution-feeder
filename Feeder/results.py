...
'''
date: 1/4/2026 implement a results class because the scope is increasing. 
'''
import math
import numpy as np
...
#helper function to format voltages
...
#helper function to build rows in bus table
...
def meta_box(bus_dict, line_list: list, tolerance: float,maxiteration: int, iterations: int, converge: bool, solver = "Gauss Siedel",case = "Case"):
    ...
    print('Total Q_load consumed: {} pu'.format(formatter(q_load/s_base)))
...
def switched_pv_report(pv_meta_dict):
    if len(pv_meta_dict['switched']) == 0:
        ...
        headers = ["Bus Id", "Vspec (pu)", "Qmin","Qmax", 'Qatswtch', 'Q clamped to', "Limit Hit",'Iterations']
        rows = []
        #for loop to iterate items 
        for bus, data in pv_meta_dict['switched'].items():
            ...
            rows.append([
                ...
                data["Q_min"],
                data['Q_max'],
                data['Qcalc_at_switch']+data['Q_load'],
                data['Q_clamped_to'],
                data['limit_hit'],
                data['iter_at_switch']
            ])
